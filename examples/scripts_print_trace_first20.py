...
async def main():
    params = StdioServerParameters(command='uv', args=['run','-m','langfuse_mcp','--public-key',pk,'--secret-key',sk,'--host',host,'--log-level','INFO'])
    async with stdio_client(params) as (read, write):
        async with ClientSession(read, write) as session:
            await session.initialize()
            # Request full JSON to avoid compact truncation
            res = await session.call_tool('fetch_traces', {'age': 10080, 'limit': 1, 'page': 1, 'include_observations': True, 'output_mode': 'full_json_string'})
